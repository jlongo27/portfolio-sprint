...
export default function NewsHero({ count }: { count: number }) {
  ...
  return (
    <section ref={sectionRef} className="relative h-screen bg-[#f3f3f3] flex flex-col">
      <NavBar darkText />
      ...
      <div className="flex-1 flex flex-col justify-between pt-28 md:pt-36 pb-8">
        ...
        <div className="flex items-start justify-between px-8 max-md:px-4">
          ...
          <div ref={numRef} className="will-change-transform text-right">
            <span
              className="text-[clamp(64px,10vw,140px)] font-light not-italic leading-[0.85] tracking-[-0.08em] text-[#1f1f1f]/10"
              style={interStyle}
            >
        ...
        {/* Heading — full-width, clips at viewport edge */}
        <div
          ref={headingRef}
        ...
        <div className="px-8 max-md:px-4 flex flex-col gap-4">
          <hr ref={ruleRef} className="border-t border-black/15 m-0" />
          <div ref={bottomRef} className="flex items-end justify-between will-change-transform">
            <div className="flex gap-4 md:gap-10 text-[#1f1f1f] text-[12px] uppercase leading-[1.1]" style={monoStyle}>
              ...
              <span className="hidden md:inline">Project Launches</span>
            </div>
